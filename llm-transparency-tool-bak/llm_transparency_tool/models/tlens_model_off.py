...
class _RunInfo:
    ...
    en_subj_tokens: Int[torch.Tensor, "batch pos"]
    en_obj_tokens: Int[torch.Tensor, "batch pos"]
    demo_tokens: Int[torch.Tensor, "batch pos"]
...
class TransformerLensTransparentLlm(TransparentLlm):
    ...
    def run(
        ...
        en_subj_token = self._model.to_tokens(subject_en, prepend_bos=False)
        en_obj_token = self._model.to_tokens(object_en, prepend_bos=False)
        demo_tokens = self._model.to_tokens(demo, prepend_bos=self._prepend_bos)
        ...
        self._last_run = _RunInfo(
            ...
            en_subj_tokens=en_subj_token,
            en_obj_tokens=en_obj_token,
    ...
    def decomposed_ffn_out(
        ...
        n_tokens = self.tokens()[batch_i].shape[0]
        processed_activations = self._get_block(layer, "mlp.hook_post")[batch_i][
            n_demo_tokens:n_tokens
        ]  ########################

        return torch.mul(
            processed_activations.unsqueeze(-1), self._model.blocks[layer].mlp.W_out
        )
    ...
    def neuron_activations(
        ...
        return self._get_block(layer, "mlp.hook_pre")[batch_i][
            pos
        ]  ######################
    ...
    def attention_output_per_head(
        ...
        return self._get_block(layer, "attn.hook_result")[batch_i][pos][
            head
        ]  #####################
    ...
    def attention_output(
        ...
        return self._get_block(layer, "hook_attn_out")[batch_i][
            pos
        ]  ######################

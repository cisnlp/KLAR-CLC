...
def build_full_graph(
    ...
    # n_demo_tokens = model._last_run.demo_tokens[batch_i].shape[0] - 1 # one additional <eos> token
    # n_demo_tokens = max(0, n_demo_tokens)
    n_test_tokens = n_tokens - n_demo_tokens

    builder = GraphBuilder(n_layers, n_tokens)

    c_attns, c_resid_attns = [], []
    # TODO: only consider the test input tokens (ignore demostration)
    for layer in range(n_layers):
        ...
        # )
        c_attn, c_resid_attn = contributions.get_attention_contributions(
            resid_pre=model.residual_in(layer)[batch_i][
                n_demo_tokens:n_tokens
            ].unsqueeze(0),
            resid_mid=model.residual_after_attn(layer)[batch_i][
                n_demo_tokens:n_tokens
            ].unsqueeze(0),
            decomposed_attn=model.decomposed_attn(batch_i, layer)[
                n_demo_tokens:n_tokens
            ].unsqueeze(0),
        )
        ...
        for token_from in range(n_test_tokens):
            for token_to in range(n_test_tokens):
        ...
        for token in range(n_test_tokens):
        ...
        c_ffn, c_resid_ffn = contributions.get_mlp_contributions(
            ...
            resid_post=model.residual_out(layer)[batch_i][
                n_demo_tokens:n_tokens
            ].unsqueeze(0),
            mlp_out=model.ffn_out(layer)[batch_i][n_demo_tokens:n_tokens].unsqueeze(0),
        )
        if renormalizing_threshold is not None:
    ...
    return builder.graph, {"c_attns": c_attns, "c_resid_attns": c_resid_attns}

...
@st.cache_resource(hash_funcs={TransformerLensTransparentLlm: id})
def load_model(
    model_name: str,
    _device: str,
...
def st_placeholder(
    ...
    empty.container(border=border, height=height).write(
        f"<small>{text}</small>", unsafe_allow_html=True
    )
    return empty
